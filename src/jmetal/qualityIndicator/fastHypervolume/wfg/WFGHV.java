...
//  public double getHV1(Front front){
//    double volume = 0.0 ;
//    if (front.nPoints_ == 0){
//    	volume = 0.0;
...
//	}else{
//	    sort(front);// sort front according to m objective (max-min)
//	
//	    if (currentDimension_ == 2)
//	      volume = get2DHV(front);
//	    else {
...
// returns -1 if p dominates q, 1 if q dominates p, 2 if p == q, 0 otherwise
  ...
  int dominates2way_(Point p, Point q)
//returns -1 if p dominates q, 1 if q dominates p, 2 if p == q, 0 otherwise
...
//		front.readFront(String.format("NSGA-II_SBX\\NSGAII_%s_%d_T%d",problem_name,m,i+1), referencePoint);
    	front.readFront("FUN", referencePoint);
