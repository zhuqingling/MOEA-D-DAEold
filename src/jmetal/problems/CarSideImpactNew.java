 ...
 */
public class CarSideImpactNew extends Problem {                     
  ...
  */
  ...
  public void evaluateConstraints(Solution solution) throws JMException {
	    ...
	    constraint[0] = - ((1.16 - 0.3717* x[1] * x[3] - 0.00931 * x[1] * x[9] - 0.484 * x[2] * x[8]
	    		+ 0.01343 * x[5] * x[9] )/1.0 - 1)  ;	  

	    constraint[1] = - ((0.261 - 0.0159 * x[0] * x[1]
	    		- 0.188 * x[0] * x[7] - 0.019 * x[1] * x[6]
	    		+ 0.0144 * x[2] * x[4] + 0.87570001 * x[4] * x[9]
	    		+ 0.08045 * x[5] * x[8] + 0.00139 * x[7] * x[10]
	    		+ 0.00001575 * x[9] * x[10])/0.32 - 1);
	  
	    constraint[2] = - ((0.214 + 0.00817 * x[4] 
	    		...
	    		+ 0.00184 * x[8] * x[9] - 0.018 * x[1] * x[1])/0.32-1);
	    
	    constraint[3] = - ((0.74 - 0.61* x[1] - 0.163 * x[2] * x[7]
			    + 0.001232 * x[2] * x[9] - 0.166 * x[6] * x[8]
			    + 0.227 * x[1] * x[1])/0.32 -1);
	    constraint[4] = -((28.98 + 3.818 * x[2] 
	     		- 4.2 * x[0] * x[1] + 0.0207 * x[4] * x[9]
	     	    		+ 6.63 * x[5] * x[8] - 7.77 * x[6] * x[7]
	     	    		+ 0.32 * x[8] * x[9])/ - 1);
	   
	    constraint[5] = -((33.86 + 2.95 * x[2]
	    		+ 0.1792 * x[9] - 5.057 * x[0] * x[1]
	    		- 11 * x[1] * x[7] - 0.0215 * x[4] * x[9]
	    		- 9.98 * x[6] * x[7] + 22 * x[7] * x[8]) - 1);
	    		
	    constraint[6] = -((46.36 - 9.9 * x[1] - 12.9 * x[0] * x[7]
	    		+ 0.1107 * x[2] * x[9]) - 1);
	   
	        			    		
	    constraint[7] =  - ( (4.72 - 0.5 * x[3] - 0.19 * x[1] * x[2]
	    		- 0.0122 * x[3] * x[9] + 0.009325 * x[5] * x[9]
	    		+ 0.000191 * x[10] * x[10])/4.0 - 1);
	    
	    constraint[8] =  -((10.58 - 0.674 * x[0] * x[1] 
	    		- 1.95  * x[1] * x[7]  + 0.02054  * x[2] * x[9] 
	    		- 0.0198  * x[3] * x[9]  + 0.028  * x[5] * x[9])/9.9 - 1) ;
	    
	    constraint[9] =  -((16.45 - 0.489 * x[2] * x[6]
	    		- 0.843 * x[4] * x[5] + 0.0432 * x[8] * x[9]
	    		- 0.0556 * x[8] * x[10] - 0.000786 * x[10] * x[10])/15.7 - 1);    	
