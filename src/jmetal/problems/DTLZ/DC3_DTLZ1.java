...
public class DC3_DTLZ1 extends Problem {   
  ...
  */    
	public void evaluate(Solution solution) throws JMException {
		...
		double [] constraintm=new double[numberOfObjectives_+1];
		double constraint=0.0;
		double a=5.0,b=0.5;
		for(int i=0;i<numberOfObjectives_;i++) {
			constraintm[i]=Math.cos(a*Math.PI*x[i])-b;
		}
		constraintm[numberOfObjectives_]=Math.cos(a*Math.PI*g)-b;
		for(int i=0;i<numberOfObjectives_+1;i++) {
			if (constraintm[i] < 0.0)
				constraint=constraint-constraintm[i];
		}
